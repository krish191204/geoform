    ...
    for (const c of world.cities) {
      ...
      if (slope > 0.35) {
        issues.push({
          ...
          critique: 'Slope under this settlement is brutal. Pre-modern cities cling to gentler ground near water.',
          fix: 'Slide it downhill toward a river terrace or bay.',
          at: { x: x / w, y: y / h },
          confidence: 0.8,
      ...
      if (world.temp && world.temp[i] < 0.12 && elev[i] > 0.7) {
        issues.push({
          ...
          critique: 'Cold, high, and harsh — possible as a fortress, weak as a capital.',
          fix: 'Keep high sites military; put population in milder valleys.',
          at: { x: x / w, y: y / h },
          confidence: 0.7,
        })
      }
      ...
      if (world.flux) {
        ...
        if (!nearWater) {
          issues.push({
            ...
            critique: 'No coast or strong stream nearby. Large settlements usually need reliable water.',
